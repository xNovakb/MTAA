    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 Vybavené OK")
                return
        else:
            now = int(time.time())
        ...
        logging.info("Bol zaregistrovany novy pouzivatel: %s [Adresa: %s]" % (fromm,contact))
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 Momentálne som preč")
        else:
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Neprijateľné")
        else:
            self.sendResponse("500 Server Internal Error")
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                if "20 INVITE" in data[5] and "Ringing" in data[0]:
                    logging.info("Zvonenie %s -> %s", self.getOrigin(), self.getDestination())
                elif "21 INVITE" in data[5] and "Ok" in data[0]:
                    logging.info("Videohovor %s -> %s", self.getOrigin(), self.getDestination())
                elif "INVITE" in data[5] and "Ok" in data[0]:
                    logging.info("Zaciatok hovoru %s -> %s", self.getOrigin(), self.getDestination())
                elif "BYE" in data[5] and "Ok" in data[0]:
                    logging.info("Koniec hovoru %s -> %s", self.getOrigin(), self.getDestination())
                elif "INVITE" in data[5] and "Decline" in data[0]:
                    logging.info("Zamietnuty hovor %s -> %s", self.getOrigin(), self.getDestination())
